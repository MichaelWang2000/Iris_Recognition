...
class Ui_adLogin1(object):
    def setupUi(self, Dialog):
        ...
        self.textEdit.setGeometry(QtCore.QRect(210, 100, 231, 51))
        self.textEdit.setObjectName("textEdit")
        self.textEdit_2 = QtWidgets.QLineEdit(parent=Dialog)
        self.textEdit_2.setGeometry(QtCore.QRect(210, 200, 231, 51))
        self.textEdit_2.setObjectName("textEdit_2")
        self.label = QtWidgets.QLabel(parent=Dialog)
        self.label.setGeometry(QtCore.QRect(70, 110, 58, 41))
        self.label.setObjectName("label")
        self.label_2 = QtWidgets.QLabel(parent=Dialog)
        self.label_2.setGeometry(QtCore.QRect(70, 210, 58, 41))
    ...
    def notExistID(self):
        box = QtWidgets.QMessageBox()
        box.warning(self, "提示", "当前管理员账户不存在")

    # 判断是否存在当前管理员账号
    def notexistID(self):
        db = getDatabase()
        cursor = db.cursor()
        sql = "select * from administrator where ad_account = "+repr(self.textEdit.toPlainText())+""
        cursor.execute(sql)
    ...
    def wrongPassword(self):
        box = QtWidgets.QMessageBox()
        box.warning(self, "提示", "管理员密码错误")

    # 判断当前账号的密码是否正确
    def Correctpassword(self):
        db = getDatabase()
        cursor = db.cursor()
        sql = "select ad_password from administrator where ad_account = " + repr(self.textEdit.toPlainText()) + ""
        cursor.execute(sql)
        result = cursor.fetchall()
