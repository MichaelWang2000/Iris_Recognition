...
class Ui_adLogin2(object):
    ...
    def contrast_image(self):
        ...
        QMessageBox = QtWidgets.QMessageBox()
        if ad_name != self.ad_account:
            QMessageBox.warning(self.centralwidget, "Error", "虹膜图片与当前管理员不匹配")
            return False, ad_name
        else:
            return True, ad_name

    # 获取当前管理员账户并查看是否相符
    def getadAccount(self, ad_account):
        self.ad_account = ad_account
        # print(self.ad_account)

    def UpdateInformation(self):
        db = getDatabase()
        cursor = db.cursor()

        sql = "UPDATE administrator SET  ad_LastTime = " + repr(QDateTime.currentDateTime().toString("yyyy-MM-dd hh:mm:ss")) + " where ad_account = " + self.ad_account + ""
        cursor.execute(sql)

        sql2 = "insert into ad_Time(ID, time, type) values(" + repr(self.ad_account) + "," + repr(QDateTime.currentDateTime().toString("yyyy-MM-dd hh:mm:ss"))  + ", 1)"
